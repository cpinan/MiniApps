...
const donatePos = await app.evalJs(`const d = document.querySelector('.donate').getBoundingClientRect();
...
const donateLoud = await app.evalJs(`const el = document.querySelector('.donate');
...
check('con prefers-reduced-motion se queda quieto',
  await app.evalJs("return getComputedStyle(document.querySelector('.donate')).animationName === 'none';"));
