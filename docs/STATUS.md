...
- **`apps/pokeprice/` (💰 Cotizador PokeMMO)**: entrenamiento por experiencia hasta nivel 100 y
  crianza 2×31, con tarifas editables, pedido de varias líneas y mensaje para el cliente. La
  lógica pura vive en `apps/pokeprice/exp.js` (seis curvas de gen 3+, precios y **601 especies**).
  La curva va amarrada a la especie en las dos pestañas; el tope del servicio es el nivel 100 y
  vive en una sola constante, `SERVICE_CAP`.
...
- **El texto de la cotización es un mensaje de WhatsApp**: cada línea del pedido guarda `detail`
  (técnico, para la pantalla) y `short` (para el cliente), con el rango siempre entero — "entrenar
  del 1 al 100", nunca "hasta nivel 100". Las negritas son cinco casillas en la pestaña Pedido
  (Pokémon, precio, total, adelanto, descuento), encendidas de fábrica y guardadas con el pedido;
  el subtotal se queda siempre en plano. `clean()` en `apps/pokeprice/app.js` quita del mensaje
  los asteriscos escritos a mano — WhatsApp no sabe escaparlos — sin tocar lo que se ve en
  pantalla.
...
Nada en vuelo. Árbol limpio, `4733193` pusheado a `main` y **`v1.4.0` tageado y subido** (el
CHANGELOG ya no tiene sección "Sin publicar").
...
- El precio de "entregado entrenado" **sube lo que se le cobra al cliente** por ese extra. Una
  cotización vieja en papel ya no cuadra con la app.
- **El mensaje de WhatsApp no está probado en un teléfono de verdad**: los asteriscos están
  verificados por test, pero nadie ha pegado todavía una cotización en un chat real para mirar
  cómo queda.

## No repetir

- **No hay red en las sesiones de esta máquina** para HTTP: `curl` a PokeAPI falla con código 000,
  incluso fuera del sandbox. (El `git push` por SSH a GitHub sí funciona.) Los datos de Pokémon
  salen de las ROMs del cliente instalado (arriba) o de
...
- **No meter el `×2` ni el "c/u" dentro de los asteriscos** del mensaje: WhatsApp no cierra la
...
- **La primera corrida de `tools/verify.sh` puede fallar sola** con
  `Failed to update a ServiceWorker … An unknown error occurred when fetching the script`. Es una
  carrera del service worker con el servidor de test, no una regresión: volver a correrla.
